...
Deno.serve(async (req) => {
  try {
    ...
    const [accountsPayable, accountsReceivable, documents, employees, employeeContracts, investments, materialRequisitions] = await Promise.all([
      base44.entities.AccountPayable.list('', 1000),
      base44.entities.AccountReceivable.list('', 1000),
      base44.entities.Document.list('', 1000),
      base44.entities.Employee.list('', 500),
      base44.entities.EmployeeContract.list('', 500),
      base44.entities.Investment.list('', 500),
      base44.entities.MaterialRequisition.list('', 500),
    ]).catch(() => [[], [], [], [], [], [], []]);
    ...
    console.log('Destinatários:', config.whatsapp_recipients);

    return Response.json({
      success: true,
      message: 'Alerta preparado com sucesso',
      preview: mensagem,
      recipients: config.whatsapp_recipients.length
    });
  } catch (error) {
